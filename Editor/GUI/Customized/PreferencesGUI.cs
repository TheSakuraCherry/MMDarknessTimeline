...
namespace MMDarkness.Editor
{
    public class PreferencesGUI : ICustomized
    {
        public virtual void OnGUI()
        {
            ...
            GUILayout.BeginVertical("box");
            var lan = EditorTools.CleanPopup("Language", Lan.Language, Lan.AllLanguages.Keys.ToList());
            if (lan != Lan.Language) Lan.SetLanguage(lan);
            ...
            if (Prefs.timeStepMode == Prefs.TimeStepMode.Seconds)
                Prefs.snapInterval = EditorTools.CleanPopup(Lan.PreferencesSnapInterval, Prefs.snapInterval,
                    Prefs.snapIntervals.ToList());
            else
                Prefs.frameRate =
                    EditorTools.CleanPopup(Lan.PreferencesFrameRate, Prefs.frameRate, Prefs.frameRates.ToList());
            ...
            Prefs.savePath = EditorTools.GUILayoutGetFolderPath(Lan.PreferencesSavePath, Lan.PreferencesSavePathTips,
                Prefs.savePath, true);
            Prefs.SerializeSavePath =
                EditorTools.GUILayoutGetFolderPath(Lan.SerializeSavePath, "序列化文件保存地址", Prefs.SerializeSavePath, true);
        ...
        {
            if (GUILayout.Button(Lan.PreferencesHelpDoc)) Application.OpenURL("https://nobug.cn/book/414447506088261");
        }
    }
}