...
namespace UnityEditor
{
    ...
    internal class PreviewResizer
    {
        ...
        private int id
        {
            get
            {
                if (m_Id == 0) m_Id = GUIUtility.GetControlID(m_ControlHash, FocusType.Passive, default);
        ...
        {
            if (m_ControlHash != 0 && !string.IsNullOrEmpty(m_PrefName)) return;
        ...
        {
            return ResizeHandle(windowPosition, minSize, minRemainingSize, resizerHeight, default);
        }

        public float ResizeHandle(Rect windowPosition, float minSize, float minRemainingSize, float resizerHeight,
            Rect dragRect)
        {
            if (Mathf.Abs(m_CachedPref) < minSize) m_CachedPref = minSize * Mathf.Sign(m_CachedPref);

            var num = windowPosition.height - minRemainingSize;
            var flag = GUIUtility.hotControl == id;
            var num2 = !flag ? Mathf.Max(0f, m_CachedPref) : s_DraggedPreviewSize;
            var flag2 = m_CachedPref > 0f;
            var num3 = Mathf.Abs(m_CachedPref);
            var position = new Rect(0f, windowPosition.height - num2 - resizerHeight, windowPosition.width,
                resizerHeight);
            ...
            if (flag) s_DraggedPreviewSize = num2;

            if (num2 < minSize) num2 = num2 >= minSize * 0.5f ? minSize : 0f;

            if (flag2 != flag3) num2 = !flag2 ? 0f : num3;
        ...
        {
            if (GUIUtility.hotControl == id) return s_CachedPreviewSizeWhileDragging > 0f;
        ...
        {
            if (GUIUtility.hotControl == id) return Mathf.Max(0f, s_CachedPreviewSizeWhileDragging);
            ...
            switch (current.GetTypeForControl(id))
            {
                ...
                case EventType.MouseUp:
                    if (GUIUtility.hotControl == id)
                    {
                        ...
                        if (!s_MouseDragged) expanded = !expanded;
